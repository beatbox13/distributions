...
class StartWindow(tk.Frame):        # class inherits from tk.Frame method to create a new window frame
    ...
    def __init__(self, parent, controller):
        ...
        def dist_choice():
            if check_var0.get() == 1 and check_var1.get() == 0 and check_var2.get() == 0 and check_var3.get() == 0 and check_var4.get() == 0 and check_var5.get() == 0 and check_var6.get() == 0:
                controller.show_frame(ParamUniform)
        ...
        start_button= tk.Button(self, text= "Start", font= ("Courier", 100), fg= "green", command= dist_choice)
        quit_button= tk.Button(self, text= "Quit ", font= ("Courier", 100), fg= "red", command= quit)
        # command= lambda: controller.show_frame(ParamUniformTriangularNormalExponentialErlangGammaWeibull)
...
class GraphWindow(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label_txt= tk.Label(self, text="Sample Distribution", anchor="center", justify="left", pady=30, font=("Courier", 18))
        label_txt.grid(row= 0, column= 0, pady= 10, padx= 10)
        #label_txt.grid(row=1, column=0, padx=10, pady=10, sticky= "nsew")


        string_var0= tk.StringVar()     # tkinter requires text entry strings to be converted using the tk.StringVar method for them to be able to be passed to other functions

        low_field= tk.Entry(self, textvariable= string_var0, width= 5)     # text entry field for user specified parameters

        low_field.grid(row= 1, column= 1, sticky= "n")
